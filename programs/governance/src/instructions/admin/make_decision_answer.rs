...
pub struct MakeDecisionAndExecuteAnswer<'info> {
    #[account(
...
pub fn make_decision_answer(
    ...
    require!(
        decision_vote.count_success == decision_vote.count_adjourn,
        GovernanceError::VoteCountNotEqual
    );

    // Always set result as Success
    governance_item.decision_result = DecisionResult::Success;
    decision_vote.finalized = true;

    // Set answer time and initialize answer vote
    ...
    emit!(crate::instructions::admin::set_decision_answer::AnswerStarted {
        quest_key,
        created_at: current_time as u64,
        end_at: end_time as u64,
    });

    emit!(DecisionResultMade {
        quest_key,
