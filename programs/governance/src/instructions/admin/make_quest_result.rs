...
pub struct MakeQuestResult<'info> {
    #[account(
...
pub fn make_quest_result(ctx: Context<MakeQuestResult>, quest_key: u64) -> Result<()> {
    let config = &ctx.accounts.config;
    let governance_item = &mut ctx.accounts.governance_item;
    let quest_vote = &mut ctx.accounts.quest_vote;
    let _governance = &mut ctx.accounts.governance;
    ...
    require!(
        quest_vote.count_approver == quest_vote.count_rejector,
